...
class Study(object):
    ...
    def _estimateLikelihoodCacheSize(self):
        """
        ...
        """
        return len(self.formattedData) * int(np.prod(self.gridSize)) * np.dtype(float).itemsize / 2. ** 20

    def _shouldCacheLikelihoods(self, cacheLikelihoods, maxCacheSize):
        """
        Decide whether a likelihood cache should be allocated.
        """
        cacheSize = self._estimateLikelihoodCacheSize()

        if isinstance(cacheLikelihoods, (bool, np.bool_)):
        ...
        if isinstance(cacheLikelihoods, str) and cacheLikelihoods == 'auto':
            ...
            if maxCacheSize < 0:
                raise ConfigurationError('maxCacheSize must be non-negative or None.')
            return cacheSize <= maxCacheSize, cacheSize

        raise ConfigurationError('cacheLikelihoods must be True, False, or "auto".')

    def _computeLikelihoodSequence(self):
        """
        Pre-compute observation likelihoods for all formatted data segments.

        Returns:
            ndarray: Likelihood values with shape [nTime] + gridSize.
        """
        likelihoodSequence = np.empty([len(self.formattedData)] + self.gridSize, dtype=float)

        for i, dataSegment in enumerate(self.formattedData):
            ...
            likelihoodSequence[i] = likelihood

        return likelihoodSequence

    def _fitFormattedData(self, forwardOnly=False, evidenceOnly=False, silent=False,
                          cacheLikelihoods='auto', maxCacheSize=512, likelihoodSequence=None):
        """
        Run the forward-backward algorithm on already formatted data.
        """
        latticeConstant = np.prod(self.latticeConstant)

        forceCache = isinstance(cacheLikelihoods, (bool, np.bool_)) and bool(cacheLikelihoods)
        if likelihoodSequence is None and (forceCache or not (forwardOnly or evidenceOnly)):
            useCache, cacheSize = self._shouldCacheLikelihoods(cacheLikelihoods, maxCacheSize)
            if useCache:
                likelihoodSequence = self._computeLikelihoodSequence()
                if not silent:
                    print('    + Cached observation likelihoods ({:.1f} MiB).'.format(cacheSize))
        ...
        for i in enum:
            ...
            if likelihoodSequence is None:
                ...
                likelihood = likelihoodSequence[i]
            ...
            self.localEvidence[i] = norm * latticeConstant  # integration yields evidence, not only sum
        ...
        self.logEvidence += np.log(latticeConstant)  # integration yields evidence, not only sum
        if not silent:
            ...
            for i in enum:
                ...
                with np.errstate(invalid='ignore'):
                    self.localEvidence[i] = 1. / (np.sum(self.posteriorSequence[i] / likelihood) *
                                                  latticeConstant)  # integration, not only sum
    ...
    def fit(self, forwardOnly=False, evidenceOnly=False, silent=False,
            cacheLikelihoods='auto', maxCacheSize=512):
        """
        ...
        Args:
            ...
            cacheLikelihoods(bool or str): If set to True, observation likelihoods are cached for all time steps. If set
                to "auto", caching is enabled only when the estimated cache size fits into maxCacheSize. If False,
                likelihoods are computed on demand.
            maxCacheSize(float): Maximum likelihood cache size in MiB used by cacheLikelihoods="auto". Set to None for
                no limit.
        """
        ...
        self._fitFormattedData(forwardOnly=forwardOnly,
                               evidenceOnly=evidenceOnly,
                               silent=silent,
                               cacheLikelihoods=cacheLikelihoods,
...
class HyperStudy(Study):
    ...
    def fit(self, forwardOnly=False, evidenceOnly=False, silent=False, nJobs=1, customHyperGrid=False,
        ...
        Args:
            ...
            cacheLikelihoods(bool or str): If set to True, observation likelihoods are cached for all time steps and
                reused across hyper-parameter values. If set to "auto", caching is enabled only when the estimated cache
                size fits into maxCacheSize. If False, likelihoods are computed on demand.
            maxCacheSize(float): Maximum likelihood cache size in MiB used by cacheLikelihoods="auto". Set to None for
                no limit.
        """
        self.fitWarningCounter = 0
            ...
            # single process fit
                ...
                for i, hyperParamValues in enum:
                    ...
                    self._fitFormattedData(forwardOnly=forwardOnly,
                                           ...
                                           cacheLikelihoods=cacheLikelihoods,
                                           maxCacheSize=maxCacheSize,
                                           likelihoodSequence=likelihoodSequence)
    ...
    def _parallelFit(self, idx, nJobs, forwardOnly, evidenceOnly, silent, cacheLikelihoods, maxCacheSize):
        """
        ...
        Args:
            ...
            evidenceOnly(bool): If set to True, only forward pass is run and evidence is calculated. In contrast to the
                forwardOnly option, no posterior mean values are computed and no posterior distributions are stored.
            silent(bool): If set to True, no output is generated by the fitting method.
            cacheLikelihoods(bool or str): Whether to cache observation likelihoods.
            maxCacheSize(float): Maximum likelihood cache size in MiB used by cacheLikelihoods="auto".
        ...
        likelihoodSequence = None
        if len(S.hyperGridValues) > 1:
            useCache, _ = S._shouldCacheLikelihoods(cacheLikelihoods, maxCacheSize)
            if useCache:
                likelihoodSequence = S._computeLikelihoodSequence()
        ...
        for i, hyperParamValues in enum:
            ...
            # call fit method from parent class
            S._fitFormattedData(forwardOnly=forwardOnly,
                                evidenceOnly=evidenceOnly,
                                silent=True,
                                cacheLikelihoods=cacheLikelihoods,
                                maxCacheSize=maxCacheSize,
