...
class TestTwoParameterModel:
    ...
    def test_step_add2TM_2hp_prior_hyperpriors_TMprior(self):
        ...
        S.add_transition_model('T1', T1)
        S.add_transition_model('T2', T2)

        S.set_transition_model_prior([0.9, 0.1])
        ...
        np.testing.assert_allclose(S.get_current_transition_model_distribution(local=False)[1],
                                   [0.49402616, 0.50597384],
                                   rtol=1e-05, err_msg='Erroneous transition model probabilities.')

        np.testing.assert_allclose(S.get_current_transition_model_distribution(local=True)[1],
                                   [0.81739495, 0.18260505],
                                   rtol=1e-05, err_msg='Erroneous local transition model probabilities.')

        # test hyper-parameter distributions
        np.testing.assert_allclose(S.get_current_hyper_parameter_distribution('s2')[1],
        ...
        # test model evidence value
        np.testing.assert_almost_equal(S.log_evidence, -9.46900822686, decimal=5,
                                       err_msg='Erroneous log-evidence value.')
