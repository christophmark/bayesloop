...
class Study(object):
    ...
    def computePrior(self, silent=False):
        """
    ...
    def setTransitionModel(self, K, silent=False):
        ...
        if not silent:
            print('+ Transition model: {}. Hyper-Parameter(s): {}'
                  .format(K, self._unpackAllHyperParameters(values=False)))
    ...
    def fit(self, forwardOnly=False, evidenceOnly=False, silent=False):
        ...
        if self.observationModel.prior is not None:
            alpha = self.computePrior(silent=silent)
        else:
        ...
        if not (forwardOnly or evidenceOnly):
            # set prior distribution for forward-pass
            if self.observationModel.prior is not None:
                beta = self.computePrior(silent=True)
            else:
    ...
    def _setAllHyperParameters(self, x):
        ...
        Args:
            x: list of values (e.g. from _unpackSelectedHyperParameters)
        """
...
class OnlineStudy(HyperStudy):
    ...
    def plotParameterEvolution(self, name, color='b', gamma=0.5, **kwargs):
        ...
        Study.plotParameterEvolution(self, name, color='b', gamma=0.5, **kwargs)
